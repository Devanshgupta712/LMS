...
async def delete_user(
    ...
    await db.execute(Course.__table__.update().where(Course.trainer_id == user_id).values(trainer_id=None))
...
async def debug_columns(table_name: str, db: AsyncSession = Depends(get_db)):
    try:
        ...
        cols = [{"name": row[0], "type": row[1], "nullable": row[2]} for row in result.fetchall()]
        
        if not cols:
            return {"error": f"Table '{table_name}' not found or no columns."}
    ...
    try:
        ...
        for model, name, field in entities:
            try:
                ...
                report.append(f"Failed {name}: {str(ex)}")
        
        try:
            # Special case for Lead assigned_to_id
            await db.execute(Lead.__table__.update().where(Lead.assigned_to_id == user_id).values(assigned_to_id=None))
            report.append("Cleared Lead.assigned_to_id")
        except Exception as ex:
        ...
        await db.flush()
        return {"report": report, "status": "success_pre_commit"}
    except Exception as e:
