...
class Project(Base):
    ...
    github_url: Mapped[str | None] = mapped_column(String(500), name="githubUrl", nullable=True)
    batch_id: Mapped[str | None] = mapped_column(String, ForeignKey("batches.id"), name="batchId", nullable=True)
    ...
    start_date: Mapped[datetime | None] = mapped_column(DateTime, name="startDate", nullable=True)
    end_date: Mapped[datetime | None] = mapped_column(DateTime, name="endDate", nullable=True)
    max_team_size: Mapped[int] = mapped_column(Integer, name="maxTeamSize", default=4)
...
class Task(Base):
    ...
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    batch_id: Mapped[str | None] = mapped_column(String, ForeignKey("batches.id"), name="batchId", nullable=True)
    assigned_by: Mapped[str | None] = mapped_column(String, ForeignKey("users.id"), name="assignedBy", nullable=True)
    priority: Mapped[TaskPriority] = mapped_column(Enum(TaskPriority), default=TaskPriority.MEDIUM)
    status: Mapped[TaskStatus] = mapped_column(Enum(TaskStatus), default=TaskStatus.PENDING)
    due_date: Mapped[datetime | None] = mapped_column(DateTime, name="dueDate", nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, name="createdAt", server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime, name="updatedAt", server_default=func.now(), onupdate=func.now())
...
class Assignment(Base):
    ...
    batch_id: Mapped[str | None] = mapped_column(String, ForeignKey("batches.id"), name="batchId", nullable=True)
    course_id: Mapped[str | None] = mapped_column(String, ForeignKey("courses.id"), name="courseId", nullable=True)
    assigned_by: Mapped[str | None] = mapped_column(String, ForeignKey("users.id"), name="assignedBy", nullable=True)
    total_marks: Mapped[int] = mapped_column(Integer, name="totalMarks", default=100)
    due_date: Mapped[datetime | None] = mapped_column(DateTime, name="dueDate", nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, name="createdAt", server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime, name="updatedAt", server_default=func.now(), onupdate=func.now())
