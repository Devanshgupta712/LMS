                        ...
                        {jobs.map((j: any) => (
                            <div key={j.id} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div style={{ flex: 1, height: '24px', background: 'rgba(255,255,255,0.05)', borderRadius: '6px', overflow: 'hidden' }}>
                                    <div style={{ width: `${totalApps > 0 ? (j.application_count / totalApps) * 100 : 0}%`, height: '100%', background: 'linear-gradient(90deg, #6366f1, #8b5cf6)', borderRadius: '6px', minWidth: j.application_count > 0 ? '20px' : '0' }} />
                                </div>
                                <span style={{ width: '40px', fontSize: '14px', fontWeight: 600, color: '#a5b4fc', textAlign: 'right' }}>{j.application_count}</span>
                            </div>
                        ))}
                    </div>
                )}
